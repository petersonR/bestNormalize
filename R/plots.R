...
plot.bestNormalize <- function(x, inverse = FALSE, bounds = NULL, 
  ...
  if(is.null(bounds)) {
    xx <- seq(min(xvals, na.rm = TRUE), max(xvals, na.rm = TRUE), length = 1000)
  } else 
  ...
  if(is.null(methods)) {
    ...
    ys <- lapply(x$other_transforms, function(obj) {
      predict(obj, newdata = xx, inverse = inverse, warn = FALSE)
    })
  } else {
  ...
  if(!inverse) 
    points(x = jitter(xvals), y = rep(min(range(ys, yy, na.rm = TRUE)), length(xvals)), pch = '|')
  else 
...
plot.orderNorm <- function(x, inverse = FALSE, bounds = NULL, ...) {
  ...
  if(is.null(bounds)) {
    ...
    xx <- seq(min(bounds, na.rm = TRUE), max(bounds, na.rm = TRUE), length = 100)
