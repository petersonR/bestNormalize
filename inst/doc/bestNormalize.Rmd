...
The ORQ normalization technique uses the following transformation:
...
The reason for the shifted logit extrapolation is that it ensures that the function is 1-1, and can handle data outside the original (observed) domain. However, this approximation should be relatively minimal since we should not expect to see many observations outside the observed range if the sample size is large enough. 

The ORQ technique will not guarantee a normal distribution in the presence of ties, but it still could yield the best normalizing transformation when compared to the Box Cox, Yeo Johnson, or Lambert W x F approaches.
...
The `bestNormalize` function will save the estimated normality statistics that it obtained for each fold and repeat of cross-validation. Users can visualize these relatively easily via a boxplot (see below), which may give some insight into how much the tranformation is truly prefered by the normality statistic, or if it doesn't matter so much as much which transformation is chosen. In this example, Box-Cox, LambertW, and ORQ seem to be doing similarly to eachother, whereas the Yeo-Johnson is under-performing.
...
The `bestNormalize` function can be quite slow, since it's using repeated CV to estimate the out-of-sample efficacy. If the user specifies `out_of_sample = FALSE"`, this will highly speed up the process, however the ORQ normalization will virtually always be chosen. If this method is chosen, we recommend setting `allow_orderNorm = FALSE` in order to pick the best of the other transformations.

```{r}
bestNormalize(x, allow_orderNorm = FALSE, out_of_sample = FALSE)
```

We also make it easy for users to parallelize the repeated CV process via the `cluster` argument and the `parallel` package. 
...
Note that `orderNorm` throws an error due to the ties in the price vector. We can see that the estimated normality statistic for the OrderNorm transformation is close to one, however, so we know it is performing quite well. It is also performing better than all of the other transformations. 
...
Here we see something peculiar; none of the normalizing transformations performed will according to the normality statistics. The frequency of ties in this case makes it very difficult to find a normalizing transformation (see figure below as to why this is). None the less, orderNorm is chosen as it has the lowest estimated P/df statistic. 
